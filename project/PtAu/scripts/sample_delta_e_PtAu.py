...
Pt-Au copy of project/scripts/sample_delta_e.py (no in-place edits of the
canonical Al-Mg driver). Differences from the Al-Mg version:
    * `pair_style eam/alloy` instead of `eam/fs` (NIST O'Brien 2017 PtAu.eam.alloy).
    * Default elements / masses switched to ("Pt", "Au") / (195.0900, 196.9665).
Everything else (sampling, CG tolerances, parsing, checkpointing) is identical.

Direction matches Wagih's stored database entry
`Pt/Au_2017--OBrien-...--Pt-Au/Pt_Au_20nm_GB_segregation.dump` so the resulting
spectrum can be KS-compared directly against per-site Wagih data.

Given a relaxed pure-Pt polycrystal (post-anneal LAMMPS data file), a per-atom
GB boolean mask (from `gb_identify.py`), and the Pt-Au EAM/alloy potential, this
driver:
    ...
    3. For each sampled site, writes a small LAMMPS deck that:
           ...
           pair_style eam/alloy ; pair_coeff * * <potfile> Pt Au
           set atom <site_id> type 2     # Pt -> Au
           minimize <etol> <ftol> ...
           print "SITE_ENERGY site_id=<id> pe=<pe>"
       runs LAMMPS, and parses the relaxed total energy.
    4. Computes
           ΔE_seg(i) = E_GB^Au(i) − mean(E_bulk^Au)
...
into a skew-normal fit and compare against Wagih's Pt(Au) Fig 3 parameters.
...
# Defaults are Pt(1) / Au(2) — this Pt-Au copy of the validated Al-Mg setup.
_DEFAULT_ELEMENTS = ("Pt", "Au")
_DEFAULT_MASSES = (195.0900, 196.9665)
...
def compute_delta_e_spectrum(
    ...
    *,
    ...
    """Sample GB + bulk sites, run per-site Pt→Au CG relaxation, return ΔE.

    Parameters
    ----------
    annealed_data_file : str or Path
        Post-anneal pure-Pt polycrystal (LAMMPS data file, atom_style atomic,
    ...
    potential_file : str or Path
        Absolute path to the Pt-Au EAM/alloy file (NIST O'Brien 2017
        PtAu.eam.alloy is the validated choice).
    n_gb_sites : int
        How many GB atoms to sample.
    n_bulk_refs : int
        How many bulk reference atoms to sample and average.
    solute_type : int
        LAMMPS atom type for Au (default 2 to match our data file convention).
    bulk_separation : float
        Minimum distance (Å) from a bulk ref atom to the nearest GB atom.
        Default 8 Å ≈ 2 NN shells in FCC Pt; chosen to avoid the GB's
...
def _write_site_deck(
    *,
    ...
    elements / masses default to Pt / Au; pass e.g. ("Cu","Ni"),
    (63.546, 58.6934) for a Cu(Ni) run. Element symbols must appear as
    element blocks in the EAM/alloy file (LAMMPS matches by name, not
    by position, for eam/alloy).
    """
    el1, el2 = elements
    m1, m2 = masses
...
def _cli() -> None:
    ...
    p.add_argument("--potential", required=True, help="Pt-Au EAM/alloy file (e.g. PtAu.eam.alloy)")
    ...
    p.add_argument("--elements", default="Pt Au",
                   help='Element symbols for type 1 / type 2. For eam/alloy, '
                        'these are matched by name against blocks in the file '
                        '(file order does not have to match these args).')
    p.add_argument("--masses", default="195.0900 196.9665",
