#!/bin/bash
# Preseg restart at T=500 K, X_c=0.30 (random-IC sweep failed kinetically
...
#SBATCH --job-name=hmc_AlMg_T500_Xc0.30_preseg
...
PRESEG=/cluster/scratch/cainiu/production_AlMg_200A/poly_AlMg_200A_preseg_Xc0.30.lmp

if [ ! -f "$PRESEG" ]; then
  echo "ERROR: pre-segregated data file not found: $PRESEG" >&2
  echo "Build it with: python scripts/pre_segregate.py --xc 0.30 ..." >&2
  exit 1
fi
...
PROD_PS=200
SWAPS_PER_CALL=100
OUTSTUB="hmc_T${T%.*}_Xc0.30_preseg"
