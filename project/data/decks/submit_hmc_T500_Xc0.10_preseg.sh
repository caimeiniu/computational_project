#!/bin/bash
# Preseg restart at T=500 K, X_c=0.10 (random-IC sweep failed kinetically
...
#SBATCH --job-name=hmc_AlMg_T500_Xc0.10_preseg
...
if [ ! -f "$PRESEG" ]; then
  ...
  echo "Build it with: python scripts/pre_segregate.py --xc 0.10 ..." >&2
  exit 1
fi
