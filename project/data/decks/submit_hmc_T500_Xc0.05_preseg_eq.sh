#!/bin/bash
# Equilibration sweep job (2026-05-05): T=500 K, X_c=0.05.
# Re-runs from canonical preseg IC (X_GB(0)=0.30) using v2 deck which
# writes a checkpoint every 5 ps. Replaces the 63-ps verify-preseg run
# (TIMEOUT) with a full 300 ps PROD trajectory.
#
# 16-rank for parallel throughput (33 % public QOS quota; 3 jobs can
# run concurrently). 24 h budget (normal.24 cap); ~10 ps/h on 16 ranks
# yields ~240 ps PROD before TIMEOUT — first checkpoint enables resume.

#SBATCH --job-name=hmc_AlMg_T500_Xc0.05_preseg_eq
...
SWAPS_PER_CALL=100
RESTART_PS=5
OUTSTUB="hmc_T${T%.*}_Xc0.05_preseg_eq"
FD_PRED=0.2282
