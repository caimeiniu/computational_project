#!/bin/bash
# Equilibration sweep job (2026-05-05): T=500 K, X_c=0.075 continuation.
# Continues from data/snapshots/hmc_T500_Xc0.075_preseg_final.lmp
# (X_GB at end of prior 300 ps PROD = 0.2543, gap = -0.046 vs canon-FD
# 0.3007). Goal: descend further to plateau.
#
# 16-rank, 24 h budget, v2 deck with 5-ps checkpoints. Auto-post-process
# yields output/hmc_T500_Xc0.075_eq_cont.{json,png} + _final.lmp.

#SBATCH --job-name=hmc_AlMg_T500_Xc0.075_eq_cont
...
mkdir -p "$RUN_DIR"
cd "$RUN_DIR"
ln -sf "$SNAPSHOT" $(basename "$SNAPSHOT")
...
SWAPS_PER_CALL=100
RESTART_PS=5
OUTSTUB="hmc_T${T%.*}_Xc0.075_eq_cont"
FD_PRED=0.3007

echo "=========================================================="
echo "Job ID         : $SLURM_JOB_ID"
echo "Variant        : equilibration sweep continuation, 16-rank, v2 deck"
echo "T              : $T K"
echo "X_c            : $XC"
...
echo "Initial Mg     : prior _final.lmp (X_GB(prior end)=0.2543)"
