#!/bin/bash
# FD-seeded HMC run (2026-05-05): T=500 K, X_c=0.30.
#
# Initial condition: poly_AlMg_200A_fdseed_T500K_Xc0.3.lmp from
# build_fdseed_inits.py, which places X_GB(0) = X_GB^FD = 0.5337 at
# random GB sites and the remaining Mg at random bulk sites
# (X_bulk(0) = 0.2462). Replaces the cancelled eq_cont job 65430302,
# which would have started from X_GB(prior end) = 0.8379 (+0.30
# above FD).
#
...
#SBATCH --job-name=hmc_AlMg_T500_Xc0.30_fdseed
...
SNAPSHOT=$RUN_DIR/poly_AlMg_200A_fdseed_T500K_Xc0.3.lmp
...
SWAPS_PER_CALL=100
RESTART_PS=5
OUTSTUB="hmc_T500_Xc0.30_fdseed"
FD_PRED=0.5337
