#!/bin/bash
# Equilibration sweep job (2026-05-05): T=500 K, X_c=0.30 continuation.
# Continues from data/snapshots/hmc_T500_Xc0.30_preseg_final.lmp
# (X_GB at end of prior 300 ps PROD = 0.8379, gap = +0.304 vs canon-FD
# 0.5337 — vacuous UB, must descend ~0.30; one 24 h job likely insufficient,
# checkpoint enables resume).

#SBATCH --job-name=hmc_AlMg_T500_Xc0.30_eq_cont
...
SNAPSHOT=$REPO/data/snapshots/hmc_T500_Xc0.30_preseg_final.lmp
...
SWAPS_PER_CALL=100
RESTART_PS=5
OUTSTUB="hmc_T${T%.*}_Xc0.30_eq_cont"
FD_PRED=0.5337
...
echo "Initial Mg     : prior _final.lmp (X_GB(prior end)=0.8379)"
