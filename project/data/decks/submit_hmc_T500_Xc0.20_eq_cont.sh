#!/bin/bash
# Equilibration sweep job (2026-05-05): T=500 K, X_c=0.20 continuation.
# Continues from data/snapshots/hmc_T500_Xc0.20_preseg_final.lmp
# (X_GB at end of prior 300 ps PROD = 0.7942, gap = +0.327 vs canon-FD
# 0.4671 — vacuous UB, must descend ~0.33; one 24 h job likely insufficient,
# checkpoint enables resume).

#SBATCH --job-name=hmc_AlMg_T500_Xc0.20_eq_cont
...
SNAPSHOT=$REPO/data/snapshots/hmc_T500_Xc0.20_preseg_final.lmp
...
SWAPS_PER_CALL=100
RESTART_PS=5
OUTSTUB="hmc_T${T%.*}_Xc0.20_eq_cont"
FD_PRED=0.4671
...
echo "Initial Mg     : prior _final.lmp (X_GB(prior end)=0.7942)"
