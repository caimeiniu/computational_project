#!/bin/bash
# FD-seeded HMC run (2026-05-07): T=300 K, X_c=0.10. Task B of the
# critical-X_c + T-axis reframe (CHANGELOG 2026-05-07).
#
# Initial condition: poly_AlMg_200A_fdseed_T300K_Xc0.1.lmp from
# build_fdseed_inits.py, X_GB(0) = X_GB^FD(300K, 0.10) = 0.4268 at
# random GB sites (N_Mg(GB) = 38001) and remaining Mg at random bulk
# sites (N_Mg(bulk) = 9571). margin to ceiling = 0.1075.
#
# Purpose: T-axis first new data point. Compare against existing T=500 K
# X_c=0.10 fdseed (gap=-0.0734) to test whether breakdown is T-robust
# (sign-test) and whether |gap| increases at low T (kinetic descent
# magnitude visible in trajectory shape; thermodynamic |gap| comparison
# requires reaching plateau, may need resume).

#SBATCH --job-name=hmc_AlMg_T300_Xc0.10_fdseed
...
SNAPSHOT=$RUN_DIR/poly_AlMg_200A_fdseed_T300K_Xc0.1.lmp
...
SWAPS_PER_CALL=100
RESTART_PS=5
OUTSTUB="hmc_T300_Xc0.10_fdseed"
FD_PRED=0.4268
