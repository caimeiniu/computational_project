#!/bin/bash
# FD-seeded HMC run (2026-05-05): T=500 K, X_c=0.20.
#
# Initial condition: poly_AlMg_200A_fdseed_T500K_Xc0.2.lmp from
# build_fdseed_inits.py, which places X_GB(0) = X_GB^FD = 0.4671 at
# random GB sites and the remaining Mg at random bulk sites
# (X_bulk(0) = 0.1385). Replaces the cancelled eq_cont job 65430300,
# which would have started from X_GB(prior end) = 0.7942 (+0.33
# above FD).
#
...
#SBATCH --job-name=hmc_AlMg_T500_Xc0.20_fdseed
...
SNAPSHOT=$RUN_DIR/poly_AlMg_200A_fdseed_T500K_Xc0.2.lmp
...
SWAPS_PER_CALL=100
RESTART_PS=5
OUTSTUB="hmc_T500_Xc0.20_fdseed"
FD_PRED=0.4671
