#!/bin/bash
# FD-seeded HMC run (2026-05-08 evening): T=500 K, X_c=0.06.
# Headline-curve methodological cleanup batch (CHANGELOG 2026-05-08 evening).
#
# Initial condition: poly_AlMg_200A_fdseed_T500K_Xc0.06.lmp from
# build_fdseed_inits.py, X_GB(0) = X_GB^FD = 0.2604 at random GB sites
# (N_Mg(GB) = 23184) and the remaining Mg at random bulk sites
# (N_Mg(bulk) = 5359). Mass conservation: 23184 + 5359 = 28543 = X_c * N_total.
#
# Purpose: replace the existing preseg-eq upper-bound point at X_c=0.06
# (post-burnin imbalance -0.714, the most-reverse of the dilute trio,
# gap-W = -0.042 was anomalously shallow vs neighbours) with a plateau-
# pinned fdseed value so the 8-point headline curve is methodologically
# uniform (all fdseed).

#SBATCH --job-name=hmc_AlMg_T500_Xc0.06_fdseed
...
SNAPSHOT=$RUN_DIR/poly_AlMg_200A_fdseed_T500K_Xc0.06.lmp
...
SWAPS_PER_CALL=100
RESTART_PS=5
OUTSTUB="hmc_T500_Xc0.06_fdseed"
FD_PRED=0.2604
