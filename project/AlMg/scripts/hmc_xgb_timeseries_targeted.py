#!/usr/bin/env python3
"""Post-process one HMC run -- targeted-deck-aware variant of
hmc_xgb_timeseries.py.

Identical to hmc_xgb_timeseries.py except for the swap-counter parsing: the
targeted deck (data/decks/hmc_AlMg_targeted.lammps) runs TWO `fix atom/swap`
fixes and exposes f_swap_fwd[1,2] + f_swap_rev[1,2]; this script sums them
into the combined attempt/accept counters. Single-fix decks (hmc_AlMg_v2 /
hmc_AlMg_resume, which expose f_hmc[1,2]) are still handled as a fallback,
so this script works on any HMC run. Everything downstream of the counters
(X_GB(t), CI, fwd/rev decomposition -- all from the dump) is unchanged.

Inputs (paired by --stub):
  <stub>.log   thermo block with f_swap_fwd[1,2]+f_swap_rev[1,2]  OR  f_hmc[1,2]
...
def main():
    ...
    # ---- thermo: take the LAST block (production with swap-counter cols) ----
    # The targeted deck runs TWO `fix atom/swap` fixes -> f_swap_fwd[1,2] +
    # f_swap_rev[1,2]; sum them for the combined counters. Single-fix decks
    # (hmc_AlMg_v2 / hmc_AlMg_resume) expose f_hmc[1,2]. Accept either layout.
    ...
    if "f_swap_fwd[1]" in cols and "f_swap_rev[1]" in cols:
        # targeted two-fix deck: combined counter = fwd + rev
        n_att = prod[:, cols.index("f_swap_fwd[1]")] + prod[:, cols.index("f_swap_rev[1]")]
        n_acc = prod[:, cols.index("f_swap_fwd[2]")] + prod[:, cols.index("f_swap_rev[2]")]
    elif "f_hmc[1]" in cols:
        n_att = prod[:, cols.index("f_hmc[1]")]
        n_acc = prod[:, cols.index("f_hmc[2]")]
    else:
        sys.exit(f"production block missing swap-counter cols "
                 f"(need f_swap_fwd[1]+f_swap_rev[1] or f_hmc[1]): {cols}")
