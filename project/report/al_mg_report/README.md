...
Clean Al(Mg)-only workspace for writing the final report. This directory is
non-destructive: `assets/` contains symlinks to the existing source files in
`project/output`, `project/report/figures`, `project/docs`, and
`project/data/snapshots`.

## Source of truth

- Latest numerical results: `assets/data/*.json`.
- Report-ready figure candidates: `assets/figures/`.
- Methods and old explanatory notes: `assets/notes/`.
- Full history and job decisions: `assets/notes/changelog.md`.

Use this workspace instead of browsing the full repo when drafting the report.
The full repo includes PtAu, archived migration bundles, old presentation
figures, and stale headline plots that are not Al(Mg) report source material.
...
- `RESULT_SNAPSHOT_2026-06-02.md`: pre-landing handoff record.
...
- `CHECK_TOMORROW.md`: exact commands for checking the overnight continuation
  jobs and manually postprocessing timeout dumps.
- `assets/figures/`: curated figure links.
- `assets/data/`: curated data links.
- `assets/notes/`: methods and explanatory notes.
