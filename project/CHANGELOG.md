...
- `report/figures/polycrystal_geometry.png` — first committable version
  of the Methods figure right panel. Generated by
  `scripts/render_methods_polycrystal.py` from
  `/cluster/scratch/cainiu/production_AlMg_200A/poly_Al_200A_16g_annealed.lmp`
  (62 MB, mtime 2026-04-24 18:46; pure Al, post-anneal substrate, before
  any Mg insertion). Image semantics: bulk Al at 0.82 transparency in
  light grey; a-CNA non-FCC atoms (= the GB-site set used throughout
  the project, N_GB = 89,042) in steel blue. The render is real
  atomistic positions from the annealed substrate, not a schematic.
- `scripts/render_methods_polycrystal.py` — landed (was untracked WIP).
  Sole content change vs. yesterday's WIP draft: GB color changed from
  warm red (0.86, 0.27, 0.18) to steel blue (0.27, 0.46, 0.71) per user
  feedback ("红色太吓人了").
- `report/methods_walkthrough_2026-05-08.md` — 1-page talking points
  for today's team meeting Methods walk-through: section map, five
  decision hooks (Cu(Ni) base merge, selection-optimisation
  collaborator's appendix slot, HMC naming, figure layout, broken-band
  measure placement), six cite-key verification list, Overleaf paste
  prerequisites.

### Why hue-shift recolor instead of re-render

Direct re-render in OVITO Tachyon ran 67 min CPU on the login node and
was killed before completion (login-node load + single-thread CPU
raytrace + 480 k transparent spheres). Public-QOS SLURM was capped at
48 CPU by the three running fdseed jobs A/B/C, so the SLURM-queue path
had a similar ~8 h ETA. Hue-shift in HSV space (red mask: H ∈ [0,0.06]
∪ [0.94,1] ∧ S > 0.20; H → 0.583, S × 0.785, V × 0.826) preserves
Tachyon shading and anti-aliasing of the existing PNG; result is
visually consistent with what a fresh render would have produced. The
.py was edited to steel blue independently so the script
self-documents the intended color and any future re-render is correct
without further edit.

### Sanity checks

- a-CNA non-FCC count gives f_GB = N_GB / N_total = 89,042 / 475,715 =
  0.187 — matches Wagih 2020-reported ranges
- Recolored PNG: 17.26 % blue pixels (2D projection fraction; tracks
  3D f_GB as expected — GB sheets project at similar fraction)

### Open observation: internal "blue patches" on the top face

The top face shows large triangular / hex blue patches inside one
grain — most likely {111} stacking-fault ribbons left by the 3 K/ps
quench (a-CNA flags HCP local stacking sequences as non-FCC). Not a
methodology issue: the project GB mask IS this a-CNA partition, and
stacking-fault sites carry their own ΔE_seg in the spectrum. Could be
verified later by (a) printing per-class counts (FCC/HCP/BCC/ICO/
OTHER) or (b) a SliceModifier cross-section render. Deferred to
weekend; not blocking Methods figure delivery.

### Files this entry

- `report/figures/polycrystal_geometry.png` — new (1.36 MB; recolored
  version of the morning's red render)
- `scripts/render_methods_polycrystal.py` — landed; GB color now
  steel-blue
- `report/methods_walkthrough_2026-05-08.md` — new
- this CHANGELOG entry

### Pending follow-ups (separate commits)

- X_c=0.30 fdseed (job 65485900) TIMEOUT'd 2026-05-08 08:11 — manual
  `hmc_xgb_timeseries.py` post-process from `.dump` is the next P0;
  panel (d) refresh to 7-pt follows
- Tasks A/B (X_c=0.04 T=500, X_c=0.10 T=300) ETA today ~19:00; Task C
  (X_c=0.10 T=700) ETA tomorrow
